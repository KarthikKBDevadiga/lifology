...
function MagazineSingle(props) {
  ...
  return (
    <div
      className={
        "flex px-4 py-6 my-5 shadow hover:shadow-lg rounded-md duration-200"
      }
    >
      <div className={"w-8/12 pr-10 flex flex-col"}>
        ...
        <div
          className={`${styles["magazine-list-single-item-description"]} text-sm my-4`}
        >
          <div
        ...
        <div
          className={
            "text-sm mt-auto text-gray-400 flex justify-between flex-col sm:flex-row pt-4"
          }
        >
          <div className={"py-3 sm:py-0"}>
            {!!magazine?.created_at &&
              formatDate(new Date(magazine.created_at))}
          </div>
          <div className={"text-black flex"}>
            ...
            />
            <span>
              {magazineBookmarked ? "Bookmark Added" : "Add to bookmark"}
            </span>
          </div>
        </div>
      </div>
...
function MagazinesList(props) {
  const { token, magazines } = props;

  return (
    <div className="p-4 pt-3 space-y-6 bg-white shadow lg:col-start-1 lg:col-span-2 sm:rounded-lg">
      <div className={""}>
        {magazines?.map((magazine) => (
          <MagazineSingle key={magazine.id} token={token} magazine={magazine} />
        ))}
      </div>
    </div>
  );
